#!/usr/bin/env bash
# setup/run.sh - Unified installation for claude-code
...
show_help() {
    cat <<'HELP'
setup - Install and configure claude-code
...
check_git_hooks() {
    [ -L "$PROJECT_DIR/.git/hooks/post-commit" ] && \
    [ -L "$PROJECT_DIR/.git/hooks/post-merge" ]
}
...
install_git_hooks() {
    ...
    for hook in post-commit post-merge; do
        ...
        if [ -e "$dst" ] && [ ! -L "$dst" ]; then
            warn "Existing $hook hook found, backing up to $hook.backup"
            mv "$dst" "$dst.backup"
        fi
...
uninstall_all() {
    ...
    rm -f "$PROJECT_DIR/.git/hooks/post-commit" "$PROJECT_DIR/.git/hooks/post-merge"
    ok "Removed git hooks"
